<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $senha = $_POST["senha"];
        if (isset($_POST["comprovante"])) $comprovante = $_POST["comprovante"]; else $comprovante = NULL; 
        if (isset($_POST["especialidade"])) $especialidade = $_POST["especialidade"]; else $especialidade = NULL; 

        $oc_usuario = mysqli_query($conexao, "SELECT * FROM usuarios WHERE usuario='$usuario'");
        $qtde_linhas = mysqli_num_rows($oc_usuario);

        if ($qtde_linhas == 0){
            $sql =  "INSERT INTO usuarios
                    (nome, usuario, email, celular, conhecimento, comprovante, especialidade, senha)
                    VALUES
                    ('$nome', '$usuario', '$email', '$celular', '$conhecimento', '$comprovante', '$especialidade', '$senha')";
            $tabela = mysqli_query($conexao, $sql);
            $resposta = "Cadastro realizado com sucesso";
        } else {
            $resposta = "Usuário já existe, escolha outro";
        }
    }
?>
<!DOCTYPE html>
...
<body>
    ...
    <main>
        ...
        <div style="text-align: center;">
            <a href="login.php" style="background-color: #325D2F; padding: .5rem; color: #F0F7DA; border-radius: 10px; text-decoration: none;">Já possui conta? Login</a>
        </div>
        <form id="form" action="cadastro.php" method="post" style="display: flex; flex-direction: column; gap: 1rem; padding: 3rem;">
            <tr>
                <td><label for="nome">Nome: </label></td>
                <td><input type="text" name="nome" id="nome" maxlength="200" required/></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="number" name="celular" id="celular" maxlength="200" required/></td>
            </tr>
            <tr>
                <td><label for="email">Email: </label></td>
                <td><input type="email" name="email" id="email" maxlength="200" required/></td>
            </tr>
            <tr>
                <td><label for="conhecimento">Nível de conhecimento (amador, estudante ou profissional): </label></td>
                <select name="conhecimento" id="conhecimento" required>
            ...
            <tr>
                <td><input type="submit" value="Cadastrar-se" style="padding: 10px 24px; width: 100%;"></td>
            </tr>
            <tr>
                <td>
...
<script>
    ...
    conhecimento.addEventListener('click', function() {
        if (conhecimento.value == "estudante") {
            com.innerHTML = `
                <label for="comprovante">Declaração de matrícula: </label>
                <input type="file" name="comprovante" id="comprovante" required/>`;
            esp.innerHTML = "";
        } else if (conhecimento.value == "profissional") {
            com.innerHTML = `
                <label for="comprovante">Currículo latte: </label>
                <input type="file" name="comprovante" id="comprovante" required/>`;
            esp.innerHTML = `
